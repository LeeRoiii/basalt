...
router.delete('/trash', async (req: Request, res: Response) => {
    ...
    const { error } = await supabase.from('folders').delete().not('deleted_at', 'is', null).eq('user_id', user_id);
...
router.delete('/:id', async (req: Request, res: Response) => {
    const user_id = (req as any).user.id;
    const { error } = await supabase.from('folders').update({ deleted_at: new Date().toISOString() }).eq('id', req.params.id).eq('user_id', user_id);
    if (error) return res.status(400).json({ error: error.message });
...
router.put('/:id/restore', async (req: Request, res: Response) => {
    const user_id = (req as any).user.id;
    const { error } = await supabase.from('folders').update({ deleted_at: null }).eq('id', req.params.id).eq('user_id', user_id);
    if (error) return res.status(400).json({ error: error.message });
...
router.delete('/:id/permanent', async (req: Request, res: Response) => {
    ...
    await supabase.from('notes').update({ folder_id: null }).eq('folder_id', req.params.id).eq('user_id', user_id);

    const { error } = await supabase.from('folders').delete().eq('id', req.params.id).eq('user_id', user_id);
    if (error) return res.status(400).json({ error: error.message });
